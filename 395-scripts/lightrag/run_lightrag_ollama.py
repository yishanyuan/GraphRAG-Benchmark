...
async def main():
    ...
    if args.mode == "API" and not os.getenv("LLM_API_KEY") and not args.llm_api_key:
        print(
            "Error: LLM_API_KEY environment variable is not set and --llm_api_key is not provided. "
            "Please set one of them before running in API mode."
        )
        return
    ...
    try: #to build graph uncomment the below code
        ...
        for i, c in enumerate(corpus):
            ...
            # Preventing Ollama from failing
            ctx = ctx[:4000]  

            try:
                await rag.ainsert(ctx)
            except Exception as e:
                print(f"Error inserting context id {c.get('id') or i}: {e}")
        ...
        if not isinstance(questions, list):
            raise ValueError(f"Unexpected questions format: {type(questions)}")
        ...
        if args.sample is not None:
            sample_n = min(args.sample, total_q)
            questions_iter = questions[:sample_n]
            print(f"Using sample={sample_n} questions")
        else:
            questions_iter = questions
            print("No sampling; using all questions")


        out_dir = os.path.join(args.base_dir, "results")
        os.makedirs(out_dir, exist_ok=True)
        out_file = os.path.join(out_dir, f"{args.subset}_results_naive.json")
